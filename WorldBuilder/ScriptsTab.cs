    ...
    public partial class Form1 : Form {
        ...
        private async void SaveScript_Click(object sender, EventArgs e) {
            if (!IsEmpty(scriptIdValue.Text) && !IsEmpty(scriptValue.Text) && !ScriptError) {
                ...
                if (scriptTypeValue.SelectedItem.ToString() == "Roslyn")
                {
                ...
                var collection = MongoUtils.MongoData.GetCollection<TriggerScript>("Scripts", (string)scriptTypesValue.SelectedItem);
                await collection.ReplaceOneAsync<TriggerScript>(s => s.Id == scriptIdValue.Text, newScript, new ReplaceOptions { IsUpsert = true });
        ...
        private void LoadScript_Click(object sender, EventArgs e) {
            if (!IsEmpty(scriptIdValue.Text) && !IsEmpty(scriptTypesValue.Text)) {
                var collection = MongoUtils.MongoData.GetCollection<BsonDocument>("Scripts", (string)scriptTypesValue.SelectedItem);
                var result = MongoUtils.MongoData.RetrieveObject<BsonDocument>(collection, s => s["_id"] == scriptIdValue.Text);
                var script = TriggerScriptFactory.GetScript(result);
        ...
        private void ScriptFilterRefresh_Click(object sender, EventArgs e) {
            if (ConnectedToDB) {
                if (!IsEmpty((string)scriptFilterTypeValue.SelectedItem)) {
                    ...
                    IEnumerable<BsonDocument> result = new List<BsonDocument>();
                    
                    var collection = MongoUtils.MongoData.GetCollection<BsonDocument>("Scripts", (string)scriptFilterTypeValue.SelectedItem);
                    if (IsEmpty(scriptFilterValue.Text)) {
                        result = MongoUtils.MongoData.RetrieveObjects<BsonDocument>(collection, s => true);
                    }
                    else {
                        result = new List<BsonDocument>() { MongoUtils.MongoData.RetrieveObject<BsonDocument>(collection, s => s["_id"].AsString == scriptFilterValue.Text) };
                    }

                    foreach (var script in result) {
                        this.scriptDatabaseValues.Items.Add(script["_id"].AsString);
                    }
                }
                else {
		...
		private void TestScript_Click(object sender, EventArgs e) {
			...
			if (scriptTypeValue.Text == "Lua") {
				...
				TriggerScriptMethods host = new TriggerScriptMethods();
