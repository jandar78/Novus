...
namespace Commands
{
    public partial class CommandParser
    {
        ...
        public static void Move(IUser player, List<string> commands)
        {
            ...
            {
                InstigatorID = player.UserID.ToString(),
                InstigatorType = player.Player.IsNPC ? ObjectType.Npc : ObjectType.Player
            };
            ...
            if (!player.Player.InCombat)
            {
                ...
                foreach (var exit in room.RoomExits)
                {
                    if (exit.AvailableExits.ContainsKey(directionToGo))
                    {
                        ...
                        {//uh-oh there's a door and it's closed
        ...
        private async static void OpenContainer(IUser player, List<string> commands)
        {
            ...
            if (position.Count() > 1)
            {
                ...
                itemNameToGet = position[0];
            }

            int index = 1;
            IRoom room = Room.GetRoom(location);
            if (room != null)
            {//player didn't specify it was in his inventory check room first
                foreach (var itemID in room.GetObjectsInRoom(RoomObjects.Items))
                {
                    IItem inventoryItem = await Items.Items.GetByID(itemID);
                    inventoryItem = GetFromInnerContainer(itemNameToGet, inventoryItem, ref container, itemPosition);

                    if (inventoryItem.Name.ToLower() == itemNameToGet || inventoryItem.Name.ToLower().Contains(itemNameToGet))
                    {
                        if (index == itemPosition)
                        {
                            player.MessageHandler(((IContainer)inventoryItem).Open());
                            itemFound = true;
                            break;
                        }
                        else
            ...
            if (!itemFound)
            { //so we didn't find one in the room that matches
                var playerInventory = player.Player.Inventory.GetInventoryAsItemList(player.Player);
                foreach (IItem inventoryItem in playerInventory)
                {
                    var innerItem = GetFromInnerContainer(itemNameToGet, inventoryItem, ref container, itemPosition);
                    if (innerItem.Name.ToLower() == itemNameToGet || innerItem.Name.ToLower().Contains(itemNameToGet))
                    {
                        ...
                        if (index == itemPosition)
                        {
                            message.Self = ((IContainer)inventoryItem).Open();
        ...
        private static IItem GetFromInnerContainer(string itemName, IItem item, ref IItem container, int itemPosition = 1, int itemIndex = 1)
        {
            if (item.ItemType.ContainsKey(ItemsType.CONTAINER))
            {
                container = item;
                foreach (var innerID in ((IContainer)item).GetContents())
                {
                    IItem innerItem = Items.Items.GetByID(innerID).Result;
                    if (innerItem != null && GetFromInnerContainer(itemName, innerItem, ref container, itemPosition, itemIndex).Name.ToLower().Contains(itemName))
                    {
        ...
        private async static void CloseContainer(IUser player, List<string> commands)
        {
            ...
            {//player didn't specify it was in his inventory check room first
                foreach (var itemID in room.GetObjectsInRoom(RoomObjects.Items))
                {
                    var roomItem = await Items.Items.GetByID(itemID);
                    roomItem = GetFromInnerContainer(itemNameToGet, roomItem, ref container, itemPosition);
                    if (roomItem.Name.ToLower() == itemNameToGet || roomItem.Name.ToLower().Contains(itemNameToGet))
                    {
                        if (index == itemPosition)
                        {
                            message.Self = ((IContainer)roomItem).Close();
            ...
            { //so we didn't find one in the room that matches
                ...
                foreach (IItem inventoryItem in playerInventory)
                {
                    ...
                    if (innerItem.Name.ToLower() == itemNameToGet || innerItem.Name.ToLower().Contains(itemNameToGet))
                    {
                        ...
                        if (index == itemPosition)
                        {
                            message.Self = ((IContainer)inventoryItem).Close();
                ...
                if (!room.IsDark)
                {
                    if (door.Lockable)
                    {
                        if (door.RequiresKey)
                        {
                            ...
                            List<IItem> inventory = player.Player.Inventory.GetInventoryAsItemList(player.Player);
        ...
        //TODO: finish this.  Need to create objects in database to read from.
        ...
        private static void ChangeStance(IUser player, List<string> commands)
        {
            ...
            var stances = MongoUtils.MongoData.GetCollection<BsonDocument>("Characters", "Stances"); //this feels more like something that should be in the World database instead?
            ...
            if (player.Player.StanceState != newStance && 
                (player.Player.ActionState == CharacterActionState.None || player.Player.ActionState == CharacterActionState.Fighting)
                )
            {
        ...
        private static IDoor FindDoor(string location, List<string> commands)
        {
            ...
            {
                if ((i == 1 || i == commands.Count - 1) && dirs.Contains(commands[i]))
                { //the direction is 99.9% probably going to be at the start or end
            ...
            else if (exit == null)
            {
                ...
                if (door == null && commands.Count < 2)
                { //if the player just typed "break" we'll return a door otherwise it could be another object
                    foreach (IExit ex in exits)
                    {          //they actually wanted to break.
                        if (ex.Doors.Count > 0)
                        {
