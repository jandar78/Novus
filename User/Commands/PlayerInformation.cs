    ...
    public partial class CommandParser
    {
        ...
        private static void DisplayStats(IUser player, List<string> commands)
        {
            ...
            if (character != null)
            {
                ...
                sb.AppendLine("You are currently " + (character.ActionState != CharacterActionState.None ? character.ActionState.ToString().ToUpper() + " and " : "") + character.StanceState.ToString().ToUpper());
        ...
        {
            List<IItem> inventoryList = player.Player.Inventory.GetInventoryAsItemList(player.Player);
            StringBuilder sb = new StringBuilder();
        ...
        {
            Dictionary<Wearable, IItem> equipmentList = player.Player.Equipment.GetEquipment(player.Player);
            StringBuilder sb = new StringBuilder();
        ...
        private static void SayTo(IUser player, List<string> commands)
        {
            ...
            if (toPlayerList.Where(p => p.Player.FirstName.ToLower() == nameBreakDown[1]).Count() > 1)
        ...
        private static void Whisper(IUser player, List<string> commands)
        {
            ...
            if (commands.Count > 2)
            {
                if (commands[2].ToUpper() == "SELF")
                {
                    ...
                    message.Room = (room.IsDark ? "Someone" : player.Player.FirstName) + " whispers into " + (room.IsDark ? "their" : (player.Player.Gender.ToString() == "MALE" ? "his" : "her")) + " own shoulder.";
                }
            ...
            else
            {
                ...
                if (commands[0].Length > startAt)
                {
                    ...
                    message.Target = player.Player.FirstName + " whispers to you \"" + whisper + "\"";
                    //this is where we would display what was whispered to players with high perception?
                    message.Room = player.Player.FirstName + " whispers something to " + toPlayer.Player.FirstName;
                }
            }
        ...
        private static void Tell(IUser player, List<string> commands)
        {
            ...
            List<IActor> toPlayerList = MongoUtils.MongoData.RetrieveObjects(MongoUtils.MongoData.GetCollection<Character.Character>("Characters", "PlayerCharacter"), n => n.FirstName.ToLower() == commands[2].ToLower()).ToList<IActor>();
            toPlayerList.AddRange(MongoUtils.MongoData.RetrieveObjects(MongoUtils.MongoData.GetCollection<NPC>("Characters", "NPCCharacters"), n => n.FirstName.ToLower() == commands[2].ToLower()));
            IActor toPlayer = null;
            ...
            if (toPlayerList.Count == 0)
            {
                message.Self = "There is no one named " + commands[2].CamelCaseWord() + " to tell something.";
            }
            else if (toPlayerList.Count > 1)
            { //let's narrow it down by including a last name (if provided)
                toPlayer = toPlayerList.Where(p => p.LastName.ToLower() == commands[3].ToLower()).SingleOrDefault();
                if (toPlayer != null)
                {
            ...
            {
                toPlayer = toPlayerList.First();
                if (toPlayer.Id == player.Player.Id)
                {
            ...
            {
                message.TargetID = toPlayer.Id.ToString();
                message.InstigatorType = toPlayer.IsNPC ? ObjectType.Npc : ObjectType.Player;

                //int startAt = commands[0].ToLower().IndexOf(toPlayer.FirstName.ToLower() + " " + toPlayer.LastName.ToLower());
                //if (startAt == -1 || startAt > 11)
                //{
                //    startAt = commands[0].ToLower().IndexOf(toPlayer.FirstName.ToLower());
                //    fullName = false;
                //}

                //if (startAt > 11)
                //    startAt = 11 + toPlayer.FirstName.Length + 1;
                //else
                //    startAt += toPlayer.FirstName.Length + 1;
                //if (fullName)
                //    startAt += toPlayer.LastName.Length + 1;

                if (commands.Count > 0)
                {
                    string temp = string.Join(" ", commands);
                    message.Self = "You tell " + toPlayer.FirstName + " \"" + temp + "\"";
            ...
            if (toPlayer != null)
            {
                if (toPlayer.IsNPC)
                {
                    Character.NPCUtils.GetUserAsNPCFromList(new List<ObjectId> { toPlayer.Id })?.MessageHandler(message);
                }
                else
                {
                    Sockets.Server.GetAUser(toPlayer.Id)?.MessageHandler(message.Target);
                }
            }
        }
        ...
        private static async void Help(IUser player, List<string> commands)
        {
            StringBuilder sb = new StringBuilder();

            if (commands.Count < 3 || commands.Contains("all"))
            { //display just the names
                var cursor = await MongoUtils.MongoData.FindAll<BsonDocument>(MongoUtils.MongoData.GetCollection<BsonDocument>("Commands", "Help"));
            ...
            { //user specified a specific command so we will display the explanation and example
        ...
        public static async void ReportBug(IUser player, List<string> commands)
        {
            ...
            { //could be an internal bug being reported
