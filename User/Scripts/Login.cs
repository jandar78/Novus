...
namespace Scripts
{
    public class Login
    {
				 ...
				 switch (specificUser.currentStep) {
					 ...
					 case Steps.SUCCEEDED:
						 ...
						 if (user != null) {
							...
							specificUser.user.Player = MongoUtils.MongoData.RetrieveObject<Character.Character>("Characters", "PlayerCharacter", c => c.Id == specificUser.user.UserID);
						 }
		 ...
		 private bool ValidatePlayerPassword(ObjectId userID, string response) {
			 var found = MongoUtils.MongoData.RetrieveObjectAsync<PlayerLogin>(MongoUtils.MongoData.GetCollection<PlayerLogin>("Characters", "PlayerCharacter"), c => c._id.Equals(userID) && c.Password == response).Result;
			 if (found != null) {
				 return true;
			 }
        ...
        private bool ValidatePlayerName(ObjectId userID, string response) {
            ...
            var found = MongoUtils.MongoData.RetrieveObjectAsync<PlayerLogin>(MongoUtils.MongoData.GetCollection<PlayerLogin>("Characters", "PlayerCharacter"), c => c.FirstName.ToLower() == names[0].ToLower() && c.LastName.ToLower() == names[1].ToLower()).Result;
            if (found != null) {
                //set the id to the ID of the one we found in the database for the PlayerCharacter to match up on the Password
                usersLoggingIn.Where(u => u.user.UserID == userID).SingleOrDefault().user.UserID = found._id;
                userID = found._id;
