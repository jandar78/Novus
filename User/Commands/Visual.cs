	...
	public  partial class CommandParser {
        ...
        private static string FindAnNpc(List<string> commands, IRoom room, out bool foundIt) {
            ...
            foreach (var id in npcList) {
                ...
                if (commands[0].ToLower().Contains(result.FirstName.ToLower()) || commands[0].ToLower().Contains(result.LastName.ToLower())) {
        ...
        private static string FindAPlayer(List<string> commands, IRoom room, out bool foundIt) {
            ...
            foreach (var id in chars) {
                ...
                if (commands[0].ToLower().Contains(playerChar.FirstName.ToLower()) || commands[0].ToLower().Contains(playerChar.LastName.ToLower())) {
        ...
        private static string FindAnItem(List<string> commands, IUser player, IRoom room, out bool foundIt) {
            ...
            foreach (var id in itemsInRoom) {
                ...
                if (item.Name.ToLower() == itemName || item.Name.ToLower().Contains(itemName)) { //fuzzy logic
            ...
            if (!foundIt) { //not in room check inventory
                ...
                foreach (IItem item in inventory) {
                    if (item.Name.ToLower() == itemName || item.Name.ToLower().Contains(itemName))
                    {
                        message = item.Examine();
                        foundIt = true;
                        break;
		...
		private static void Look(IUser player, List<string> commands) {
            ...
            if (!NonAllowableStates.Contains(player.Player.ActionState)) {
                if (commands.Count > 2 && commands[2].ToLower() == "in") {
        ...
        private async static void LookIn(IUser player, List<string> commands) {
            ...
            if (!string.IsNullOrEmpty(location)) {//player didn't specify it was in his inventory check room first
                foreach (var itemID in room.GetObjectsInRoom(RoomObjects.Items)) {
                    ...
                    inventoryItem = GetFromInnerContainer(itemNameToGet, inventoryItem, ref container, itemPosition, index);

                    if (inventoryItem.Name.ToLower() == itemNameToGet || inventoryItem.Name.ToLower().Contains(itemNameToGet)) {
                        player.MessageHandler(((IContainer)container).LookIn());
                        itemFound = true;
                        break;
            ...
            if (!itemFound) { //so we didn't find one in the room that matches
                ...
                if (!itemFound)
                {
                    player.MessageHandler("That's not something you can look inside of");
                }
            }
        }
