...
namespace Commands
{
    public partial class CommandParser
    {
        ...
        public static void Drop(IUser player, List<string> commands)
        {
            ...
            if (item != null)
            {
                player.Player.Inventory.RemoveInventoryItem(item, player.Player);
                item.Location = player.Player.Location;
                item.Owner = ObjectId.Empty; //no owner
                item.Save();
        ...
        private static void Give(IUser player, List<string> commands)
        {
            ...
            if (toPlayerList.Where(p => p.Player.FirstName.ToLower() == nameBreakDown[0]).Count() > 1)
            { //let's narrow it down by including a last name (if provided)
                ...
                if (toPlayer == null)
                { //no match on full name, let's try with the dot operator if they provided one
            ...
            if (toPlayer == null)
            { //we are looking for an npc at this point
                toPlayerList.Clear();
            ...
            if (item != null && toPlayer != null)
            {
                ...
                player.Player.Inventory.RemoveInventoryItem(item, player.Player);               
                toPlayer.Player.Inventory.AddItemToInventory(item, toPlayer.Player);
        ...
        public static void Unequip(IUser player, List<string> commands)
        {
            ...
            {
                foreach (KeyValuePair<Wearable, IItem> item in player.Player.Equipment.GetEquipment(player.Player))
                {
        ...
        public static void Equip(IUser player, List<string> commands)
        {
            ...
            if (commands.Count == 2)
            {
                message.Self += "What are you trying to equip?";
            }
            //we need to make a list of items to wear from the players inventory and sort them based on stats
            else if (commands.Count > 2 && string.Equals(commands[2].ToLower(), "all", StringComparison.InvariantCultureIgnoreCase))
            {
                foreach (IItem item in player.Player.Inventory.GetAllItemsToWear(player.Player))
                {
                    if (player.Player.Equipment.EquipItem(item, player.Player))
                    {
                    ...
                    {
                        message.Self += "You can't equip that!";
                    }
                }
            }
            else
            {
                ...
                {
                    player.Player.Equipment.EquipItem(item, player.Player);
        ...
        {
            var itemName = Items.Items.ParseItemName(commands);
            int itemPosition = 1;
            string[] position = itemName.Split('.'); //we are separating based on using the decimal operator after the name of the npc/item
            if (position.Count() > 1)
            {
                int.TryParse(position[position.Count() - 1], out itemPosition);
                itemName = itemName.Remove(itemName.Length - 2, 2);
            ...
            List<IItem> items = player.Player.Inventory.GetInventoryAsItemList(player.Player).Where(i => i.Name.ToLower() == itemName || i.Name.ToLower().Contains(itemName)).ToList();
            IItem item = items[itemPosition - 1];
            ...
            {
                if (string.IsNullOrEmpty(player.Player.MainHand))
                { //no mainhand assigned yet
                    player.Player.MainHand = Wearable.WIELD_RIGHT.ToString(); //we will default to the right hand
                }

                player.Player.Equipment.Wield(item, player.Player);
                item.Save();
        ...
        private static IItem GetItem(List<string> commands, string location)
        {
            ...
            List<IItem> items = Items.Items.GetByName(itemName, ObjectId.Parse(location));
        ...
        public async static void Put(IUser player, List<string> commands)
        {
            ...
            if (location.Equals(ObjectId.Empty))
            {
                ...
                {
                    foreach (IItem tempContainer in player.Player.Inventory.GetInventoryAsItemList(player.Player))
                    {
                        containerItem = GetFromInnerContainer(containerName.CamelCaseString(), tempContainer, ref containerItem, containerPosition);
            ...
            { //player specified it is in his inventory 
                foreach (IItem tempContainer in player.Player.Inventory.GetInventoryAsItemList(player.Player))
                {
                    containerItem = GetFromInnerContainer(containerName.CamelCaseString(), tempContainer, ref containerItem, containerPosition);
                    if (string.Equals(containerItem.Name, containerName.CamelCaseString(), StringComparison.InvariantCultureIgnoreCase))
            ...
            retrievedItem = player.Player.Inventory.GetInventoryAsItemList(player.Player).Where(i => i.Name == itemName).SingleOrDefault();
        ...
        public static void Get(IUser player, List<string> commands)
        {
            ...
            List<string> commandAltered = ParseItemPositions(commands, "from", out int itemPosition, out string itemName);
            if (commands.Count >= 3)
            {
            ...
            TraverseItems(player, containerName.ToString().Trim(), itemName.ToString().Trim(), containerPosition, itemPosition, out IItem retrievedItem, out IItem containerItem);
            ...
            {              
                if (containerItem != null)
                {
                    retrievedItem = ((IContainer)containerItem).RetrieveItem(retrievedItem.Id);
                ...
                retrievedItem.Save();
                player.Player.Inventory.AddItemToInventory(retrievedItem, player.Player);
            }
            else
            {
        ...
        private static void ParseContainerPosition(List<string> commands, string separator, out int containerPosition, out string containerName)
        {
            ...
            containerName = containerNameTemp.ToString().CamelCaseString();
        }
        ...
        private static void TraverseItems(IUser player, string containerName, string itemName, int containerPosition, int itemPosition, out IItem retrievedItem, out IItem retrievedContainer)
        {
            ...
            if (!string.IsNullOrEmpty(containerName.CamelCaseString()))
            {
                foreach (var itemID in room.GetObjectsInRoom(RoomObjects.Items))
                {
                    ...
                    inventoryItem = GetFromInnerContainer(containerName.CamelCaseString(), inventoryItem, ref retrievedContainer, containerPosition, containerIndex);

                    if (inventoryItem.Name.ToLower() == itemName.ToLower() || inventoryItem.Name.ToLower().Contains(itemName.ToLower()))
                    {
            ...
            {
                var container = retrievedContainer as IContainer;
                foreach (var itemID in container.GetContents())
                {
                    IItem inventoryItem = Items.Items.GetByID(itemID).Result;

                    inventoryItem = GetFromInnerContainer(itemName.CamelCaseString(), inventoryItem, ref retrievedContainer, itemPosition, itemIndex);
            ...
            {//we are grabbing a container or an item without a specific index
                foreach (var itemID in room.GetObjectsInRoom(RoomObjects.Items))
                {
                    IItem inventoryItem = Items.Items.GetByID(itemID).Result;
        ...
        private static void Activate(IUser player, List<string> commands)
        {
            ...
            var lightItem = FindLightInEquipment(commands, player, room);
        ...
        private static IIluminate FindLightInEquipment(List<string> commands, IUser player, IRoom room)
        {
            ...
            if (commands.Count > 0)
            {
                ...
                foreach (IItem item in player.Player.Equipment.GetEquipment(player.Player).Values)
                {
                    if (item.WornOn == Wearable.WIELD_LEFT || item.WornOn == Wearable.WIELD_RIGHT)
                    {
                        var temp = item as IIluminate;
            ...
            { //let's be smart and figure out what lightSource he wants activated, first come first serve otherwise
                foreach (IItem item in player.Player.Equipment.GetEquipment(player.Player).Values)
                {
                    var lightsource = item as IIluminate;
                ...
                if (lightItem == null)
                { //not in players equipment let's check the room
                    foreach (var itemId in room.GetObjectsInRoom(RoomObjects.Items))
                    {
                        lightItem = Items.Items.GetByID(itemId).Result;
                        if (lightItem is IIluminate lightsource && lightsource.IsLightable)
                        {
                            break;
                        }
