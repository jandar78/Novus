...
namespace Rooms {
    ...
    public class Door : IDoor {
		...
		public void LoadTriggers() {
            ...
            if (Triggers != null) {
                foreach (var trigger in Triggers) {
					trigger.script = TriggerScriptFactory.GetScript(trigger.ScriptID, trigger.ScriptType);
		...
		public IDoor GetDoorFromDB() {
            ...
            return MongoUtils.MongoData.RetrieveObjectAsync<Door>(doorCollection, d => d.Id == this.Id).Result;
		}

		public IMongoCollection<Door> GetDoorCollection() {
			return MongoUtils.MongoData.GetCollection<Door>("World", "Doors");
		}
		...
		public async void UpdateDoorStatus() {
			...
			await MongoUtils.MongoData.SaveAsync<Door>(GetDoorCollection(), d => d.Id == door.Id, door);
		}
			...
			foreach (ITrigger trigger in _exitTriggers) {
				...
				if (hasOn && hasAnd) {
					if (Extensions.RandomNumber.GetRandomNumber().NextNumber(0, 100) <= trigger.ChanceToTrigger) {
						...
						trigger.HandleEvent(null, new TriggerEventArgs(ObjectId.Empty, TriggerEventArgs.IDType.Door, ObjectId.Empty, TriggerEventArgs.IDType.Door));
					}
				}
			}
		...
		public class DoorHelpers {
			...
			[LuaAccessible]
			public static async Task CreateNPC(int mobTypeID, string location, int amount) {
				amount = amount * Rooms.Room.GetRoom(location).GetObjectsInRoom(RoomObjects.Players, 100).Count;

				for (int i = 0; i < amount; i++) {
					IActor actor = await Character.NPCUtils.CreateNPC(mobTypeID);
					if (actor != null) {
