...
namespace Groups {
	...
	public class Group {
		...
		public void AssignMasterLooter(string leaderID, string playerID) {
			if (GroupRuleForLooting == GroupLootRule.Master_Looter) {
				if (string.Equals(leaderID, LeaderID)) {
					playerID = Server.GetAUserByFullName(playerID).UserID;
					if (PlayerList.Contains(playerID)) {
						MasterLooter = playerID;
						InformPlayersInGroup(string.Format("{0} has been assigned by {1} as the group Master looter.", Server.GetAUser(playerID).Player.FullName, Server.GetAUser(LeaderID).Player.FullName), new List<string>() { playerID });
		...
		public void RemoveMasterLooter(string leaderID) {
			if (string.Equals(leaderID, LeaderID)) {
				InformPlayersInGroup(string.Format("{0} is no longer the group Master looter.", Server.GetAUser(MasterLooter).Player.FullName), new List<string>() { MasterLooter });
		...
		public void RemovePlayerFromGroup(string playerID) {
			if (PlayerList.Contains(playerID)) {
				...
				InformPlayersInGroup(Server.GetAUser(playerID).Player.FullName + " has left the group.", new List<string>() { playerID });
		...
		public void PromoteToLeader(string newLeaderID) {
			if (PlayerList.Contains(newLeaderID)) {
				...
				InformPlayersInGroup(Server.GetAUser(LeaderID).Player.FullName + " has been promoted to group leader.", new List<string>(){ LeaderID });
		...
		public override string ToString() {
			...
			foreach (string playerID in PlayerList) {
				sb.AppendLine("\t" + Server.GetAUser(playerID).Player.FullName + (string.Equals(playerID, LeaderID, StringComparison.InvariantCultureIgnoreCase) ? " (Leader)" : ""));
			}
		...
		public void SayToGroup(string message, string playerToIgnore) {
			InformPlayersInGroup(Server.GetAUser(playerToIgnore).Player.FullName + " says to the group \"" + message + "\"", new List<string>(){ playerToIgnore });
