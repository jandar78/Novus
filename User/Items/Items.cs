...
namespace Items {
    public sealed partial class Items : IItem, IWeapon, IEdible, IContainer, IIluminate, IClothing, IKey {
        ...
        public List<ItemTrigger> Triggers { get; set; }
        ...
        public static string ParseItemName(List<string> commands) {
            ...
            return itemName.ToString().Trim().ToLower();
        }
        ...
        public static List<IItem> GetByName(string name, ObjectId owner) {
            ...
            var items = MongoUtils.MongoData.RetrieveObjectsAsync<Items>(collection, i => i.Name.ToLower().Contains(name) && i.Owner == owner).Result;
            List<IItem> itemList = new List<IItem>();
            foreach (var item in items) {
                itemList.Add(ItemFactory.CreateItem(item.Id).Result);
            }
        ...
        public async static void DeChargeLightSources() {
            var collection = MongoUtils.MongoData.GetCollection<Items>("World", "Items");
            var doc = await MongoUtils.MongoData.RetrieveObjectsAsync<Items>(collection, i => i.ItemType.ContainsKey(ItemsType.ILUMINATION) && i.IsLit);

            foreach (var item in doc) {
                IItem lightSource = await GetByID(item.Id);
